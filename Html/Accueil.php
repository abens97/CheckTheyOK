<!DOCTYPE html>
<html>

	<?php

	session_start();
	
    if(isset($_SESSION['email'])){
		include 'Header&Navigation&Footer.php';
	}
    else {
		include 'Header&FooterHorsConnexion.php';
	}
	
	?>

<head>
		
	
<title>Domisep</title>
<style type="text/css">
	h1 {text-shadow: 2px 14px 6px rgba(30, 64, 0, 0.5);margin-right: center; width:52%;margin:auto;font-size: 25px;text-align: center;font-family: normal;}
	.slider{margin: 25px 25%;overflow: hidden;height: 350px;width: 50%;border: 4px solid white;border-radius: 10px;box-shadow: -2px 30px 40px rgba(30, 64, 0, 0.5);}
	.slider figure div{width: 20%;float: left;}
	.slider figure img{width: 100%;height: 100%;float: left;}
	.slider figure{position: relative;width: 500%;margin: 0;left: 0;animation: slide 27s linear infinite;}
	@keyframes slide{0%{left: 0%}10%{left: 0%;}16%{left: -100%;}22%{left: -100%;}28%{left: -200%;}34%{left: -200%;}40%{left: -300%;}46%{left: -300%;}52%{left: -400%;}58%{left: -400%;}64%{left: -300%;}70%{left: -300%;}76%{left: -200%;}82%{left: -200%;}88%{left: -100%;}94%{left: -100%;}100%{left: 0%;}}
</style>


</head>

<body>
	<br><br>
	<div class="slider">
			<figure>
				<div class="slide">
					<img src="../Images/Vieux.jpg">
				</div>
		
				<div class="slide">
					<img src="../Images/Telecom.pdf">
				</div>
		
				<div class="slide">
					<img src="../Images/Telecom2.pdf">
				</div>

				<div class="slide">
					<img src="../Images/HomeFond.jpg">
				</div>
				
				<div class="slide">
					<img src="../Images/Vieux.pdf">
				</div>
			</figure>
		</div>
	<br><br>
	<h1>
 		 Faciliter le maintien à domicile des seniors ? C'est possible ! Cette maison sécurisée est La solution que vous attendiez.
	</h1>
</body>
</html>